...
object TestKafkaServer {
  ...
  /**
    * Default configurations used for consuming records using [[TestKafkaServer]].
    */
  ...
  /**
    * Default configurations used for producing records using [[TestKafkaServer]].
    */
  val defaultProducerConfig: Map[String, String] = Map(
    ProducerConfig.ACKS_CONFIG -> "all"
  )

  val defaultSerializerConfig: Map[String,AnyRef] = Map(
    "schema.registry.url" -> "https://schemaregistry.prod.judopay.com"
  )
...
/**
  ...
  */
final class TestKafkaServer(
                             val kafkaPort: Int = TestKafkaServer.randomAvailablePort(),
                             val zookeeperPort: Int = TestKafkaServer.randomAvailablePort(),
                             kafkaConfig: Map[String, String] = TestKafkaServer.defaultConfig
                           ) {

  import TestKafkaServer._

  private val logDir = createTempDir("kafka-local")

  val bootstrapServerAddress = "localhost:" + kafkaPort.toString
  ...
  def consumeAvro[Key: SchemaFor : ToRecord : FromRecord, Value: SchemaFor : ToRecord : FromRecord](
                                                                                                 topic: String,
                                                                                                 expectedNumOfRecords: Int,
                                                                                                 timeout: Long,
                                                                                                 consumerConfig: Map[String, String] = defaultConsumerConfig
    ...
    val keyDeserializer = AvroDeserializer[Key]()
    keyDeserializer.configure(defaultSerializerConfig.asJava, isKey = true)

    val valueDeserializer = AvroDeserializer[Value]()
    valueDeserializer.configure(defaultSerializerConfig.asJava, isKey = false)

    consume(topic, expectedNumOfRecords, timeout, keyDeserializer, valueDeserializer, consumerConfig)
  }
  ...
  def produce[Key, Value](
                           topic: String,
                           records: Iterable[(Key,Value)],
                           keySerializer: Serializer[Key],
                           valueSerializer: Serializer[Value]
                         ): Unit =
    produce(topic, records.map(kv => new ProducerRecord[Key,Value](topic, kv._1, kv._2)), keySerializer, valueSerializer, defaultProducerConfig)


  def produceAvro[Key: SchemaFor : ToRecord : FromRecord, Value: SchemaFor : ToRecord : FromRecord](
                                                                                                 topic: String,
                                                                                                 records: Iterable[(Key,Value)]
                                                                                               )
  : Unit = {

    val (keySerializer, valueSerializer) = buildSerializers[Key, Value]()

    produce[Key, Value](topic, records, keySerializer, valueSerializer)
  }
  ...
  : Unit = {
    ...
    produce[Key, Value](topic, records, keySerializer, valueSerializer, producerConfig)
  }

  private def buildSerializers[Key: SchemaFor : ToRecord : FromRecord, Value: SchemaFor : ToRecord : FromRecord]()
  : (Serializer[Key], Serializer[Value]) = {
    val keySerializer = AvroSerializer[Key]()
    keySerializer.configure(defaultSerializerConfig.asJava, isKey = true)

    val valueSerializer = AvroSerializer[Value]()
    valueSerializer.configure(defaultSerializerConfig.asJava, isKey = false)
